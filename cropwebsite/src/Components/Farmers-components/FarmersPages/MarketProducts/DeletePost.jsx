...
const DeletePost = ({setOpenModal, id, name}) => {
  ...
  const handleSubmit = (e) => {
    ...
    // // post forum
     ...
     const res =  api.delete(
        `/products/${id}/`, { headers:{Authorization: `Token ${token}`}}
      ).then(
        ({status, data})=>{
         if(status===204){
  ...
  return (
    <div className=' fixed top-0 left-0 h-full w-full bg-black bg-opacity-50' >
      ...
      <div className='py-3'> 
        <h1 className='text py-3 ps-3 font-semibold'>Delete Crop</h1>
        ...
        <form className='flex flex-col px-3'>
           ...
           */}
           <div>
              <h1 className='text-center'>Are you sure you want to delete this {name}?</h1>
          ...
          <button className='bg-green-700 text-white p-2 w-48 rounded shadow' type='button' onClick={handleSubmit}>Delete Crop</button>
